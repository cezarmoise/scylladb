...
class CriticalDiskUtilizationGuard {
    sharded<replica::database>& _db;
public:
    CriticalDiskUtilizationGuard(sharded<replica::database>& db) : _db(db) {
        replica::database::set_critical_disk_utilization_mode_on_all_shards(_db, true).get();
    }

    ~CriticalDiskUtilizationGuard() {
        replica::database::set_critical_disk_utilization_mode_on_all_shards(_db, false).get();
    }
};
...
SEASTAR_TEST_CASE(test_reject_user_table_writes) {
    ...
    return do_with_cql_env_and_compaction_groups([] (cql_test_env& e) {
        ...
        {
            CriticalDiskUtilizationGuard guard(e.db());
...
SEASTAR_TEST_CASE(test_reject_user_table_counter_update) {
    ...
    return do_with_cql_env_and_compaction_groups([] (cql_test_env& e) {
        ...
        const sstring ks_name = "ks";
        const sstring cf_name = "cf";
        e.execute_cql(fmt::format("CREATE TABLE {}.{} (k TEXT PRIMARY KEY, v COUNTER);", ks_name, cf_name)).get();

        auto apply_mutation = [](sharded<replica::database>& sharded_db, table_id uuid, const mutation& m) {
            auto& t = sharded_db.local().find_column_family(uuid);
            auto shard = t.shard_for_reads(m.token());
            return sharded_db.invoke_on(shard, [uuid, fm = freeze(m)] (replica::database& db) {
                auto& t = db.find_column_family(uuid);
                return db.apply_counter_update(t.schema(), fm, db::no_timeout, tracing::trace_state_ptr());
            });
        };
        ...
        auto& col = *s->get_column_definition(utf8_type->decompose(sstring("v")));
        counter_cell_builder b;
        b.add_shard(counter_shard(counter_id(utils::make_random_uuid()), 1, 1));
        m.set_clustered_cell(clustering_key_prefix::make_empty(), col, b.build(api::new_timestamp()));
