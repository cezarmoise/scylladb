...
MAX_IMBALANCE_PERCENTAGE = 5  # % maximum allowed imbalance between nodes


@pytest.mark.asyncio
@pytest.mark.nightly
@pytest.mark.parametrize("strategy", ["IncrementalCompactionStrategy"])
async def test_data_distribution_balance(manager: ManagerClient, volumes_factory: Callable, strategy: str) -> None:
    ...
    config = {
        ...
        "tablet_load_stats_refresh_interval_in_seconds": 20,  # default is 60, lowering it to make test faster
        "write_request_timeout_in_ms": 120000,  # 120s, increased from default to handle CI resource contention
    }
    cmdline = ["--logger-log-level=load_balancer=debug"]

    # Generate a topology where nodes have different disk space sizes
    topology_sizes = {"dc1": {"r1": ["150M", "200M", "300M", "400M"]}}
    ...
    async with space_limited_servers(manager, volumes_factory, topology_sizes, config=config, cmdline=cmdline) as servers:
        ...
        opts = "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 1} AND tablets = {'initial': 128}"
        async with new_test_keyspace(manager, opts) as ks:
            ...
            strategy = "IncrementalCompactionStrategy"
            schema = "pk int, ck int, v blob, PRIMARY KEY(pk, ck)"
            extra = f"WITH compaction={{'class': '{strategy}'}}"
            async with new_test_table(manager, ks, schema, extra) as cf:
                stmt = cql.prepare(f"INSERT INTO {cf} (pk, ck, v) VALUES (?, ?, ?)")
                # Will insert:
                # 1000 partitions with 100 clustering rows each
                # 100 partitions with 1000 clustering rows each
                # 10 partitions with 10000 clustering rows each
                # Total 600000 rows
                for partitions, c_rows in [(1000, 100), (100, 1000), (10, 10000)]:
                    parameters = []
                    for p in range(partitions):
                        pk = partitions + p  # avoid duplicate partition keys
                        parameters += [(pk, ck, os.urandom(1000)) for ck in range(c_rows)]

                    logger.debug(f"Will insert {partitions} partitions with {c_rows} clustering rows each, total: {len(parameters)} rows")
                    await batch_cql(cql, stmt, parameters, batch_size=500)
                ...
                # Flush everything before compaction
                await asyncio.gather(*(manager.api.flush_all_keyspaces(server.ip_addr) for server in servers))

                # Wait tablet migrations to finish, otherwise tablet cleanup will keep interrupting compaction
                mark = await log.mark()
                ...
                # Wait for final balance after compaction
                mark = await log.mark()
                await log.wait_for(r"raft_topology - raft topology: Refreshed table load stats for all DC\(s\)\.", from_mark=mark)
                await asyncio.gather(*(manager.api.quiesce_topology(server.ip_addr) for server in servers))
                ...
                for server in servers:
                    ...
                    disk_usage = round(data_bytes / capacity * 100, 2)
                    usages.append(disk_usage)

                    # debug logging for exact disk usage
                    host_id = await manager.get_host_id(server.server_id)
                    logger.debug(f"Server {server.server_id} {host_id}: {disk_usage=} ({data_bytes=} {free_bytes=} {capacity=})")
                ...
                assert balance <= MAX_IMBALANCE_PERCENTAGE, f"Data distribution is {balance} imbalanced: {usages}"
